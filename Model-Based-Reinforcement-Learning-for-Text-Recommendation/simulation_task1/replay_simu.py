...
class ReplayMemory(object):
    def __init__(self, env, policy, capacity, max_length, action_num, recom_length = 10, start_clicks = [None], evaluate=False):
        ...
        self.clicks = torch.zeros(self.capacity, max_length).type(torch.LongTensor)
        self.probs = torch.ones(self.capacity, max_length).type(torch.FloatTensor)
        self.rewards = torch.zeros(self.capacity, max_length).type(torch.FloatTensor)
        self.actions = torch.zeros(self.capacity, self.recom_num, max_length).type(torch.LongTensor)
        ...
        self.actions[:, :, 0] = torch.ones(self.capacity, self.recom_num).type(torch.LongTensor) * self.clicks[:, 0].view(-1,1)#(torch.from_numpy(self.clicks[:, 0]).view(-1,1))
    ...
    def next_click(self, click_batch, action, hidden=None, start=False):
        with torch.no_grad():
            ...
            outputk = self.env.next_click(enc_out[:,-1,:], torch.cat((action, self.env.end * torch.ones(action.size(0), 1).type(torch.LongTensor)), 1), len(click_batch))
    ...
    def gen_sample(self, batch_size):
        for stidx in range(0, self.capacity, batch_size):
            ...
            #click_batch = torch.from_numpy(click_batch)
    ...
    def write_sample(self, filename_click, filename_reward, filename_action, num_items, add_end=True): #write reward and actions
        ...
        for i in range(len(self.clicks)):
            ...
            if add_end:
                ...
                file_action.write(' ')
                self.write_action_line(file_action, (torch.ones(self.recom_num).type(torch.LongTensor) * num_items), '')
            file_action.write('\n')
            file_click.write('\n')
        file_click.close()
        file_reward.close()
