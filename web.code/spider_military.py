...
class Spider():
    ...
    def get_url(self):
        baseurl = 'http://military.china.com.cn/'
        for i in range(1, 10):
            url = baseurl + 'node_7252588_{}.htm'.format(i)
            response = requests.get(url, headers=headers)
            response.encoding = "utf-8" # 解决乱码问题
            response = response.text
            # print(response)
            html = etree.HTML(response)
            urls = html.xpath("//h3[@class='mb10 c_title']/a/@href")
            titles = html.xpath("//h3[@class='mb10 c_title']/a/text()")
