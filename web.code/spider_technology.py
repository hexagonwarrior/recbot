...
class Spider():
    ...
    def get_url(self):
        baseurl = 'https://tech.163.com/special/'
        for i in range(2, 10):
            url = baseurl + 'it_2016_0{}/'.format(i)
            response = requests.get(url, headers=headers).text
            # print(response)
            html = etree.HTML(response)
            urls = html.xpath("//div[@class='titleBar clearfix']/h3[@class='bigsize ']/a/@href")
            titles = html.xpath("//div[@class='titleBar clearfix']/h3[@class='bigsize ']/a/text()")
