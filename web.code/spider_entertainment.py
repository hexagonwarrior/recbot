...
class Spider():
    ...
    def get_url(self):
        baseurl = 'http://news.yule.com.cn/neidi/'
        for i in range(10):
            url = baseurl + 'index{}.html'.format(i)
            response = requests.get(url, headers=headers).text
            html = etree.HTML(response)
            urls = html.xpath("//div[(@class ='titname')]/a/@href")
            titles = html.xpath("//div[(@class ='titname')]/a/text()")
