...
SEQ_LEN = 48 # 点击序列长度
STATE_LEN = HISTORY_LEN + SEQ_LEN # 56 # ENV的状态长度
ACTION_LEN = 8 # 每个动作包含8个推荐的NEWS
INPUT_LEN = STATE_LEN + STATE_LEN # 64 # MODEL的input长度
...
class rec_env:
    ...
    def get_random_distributation(self, history):
        # 计算分布
    ...
    # 根据分布来采样动作
    def get_random_action(self, distribution):
        # 根据分布采样
    ...
    def get_next_states(self):
        ...
        for i in range(ACTION_NUM): # 取8个推荐做为动作
            ...
            distribution = self.get_random_distributation(history) # 计算分布
            action = self.get_random_action(distribution) # 生成8种不同分布的随机动作
            state = self.state[:] # 复制当前状态
            next_state, reward = self.step(action, state) # 模拟下一个动作，产生下一个状态
            states_list[tuple(action)] = next_state # 保存下一个状态
            
        return states_list # 反回8个动作模拟和产生的状态字典
